...
(define column-offsets '(8 5 0 6 11 84 84 11 6 0 5 8))
...
(define microcontroller-module
  `(module PROMICRO (layer Front) (tedit 4FDC31C8) (tstamp 543EF800)
     ...
     (pad D1  thru_hole circle (at -3.81 -7.62 270) (size 1.7526 1.7526) (drill 1.0922) (layers *.Cu *.SilkS *.Mask) (net 5 N-row-4))
...
(define nets
  `((net 0 "")
    ...
    ,@(for/list ([s (in-range 60)])
        (list 'net (+ 17 s) (string->symbol (format "N-diode-~s" s))))))
...
(define (switch row col)
  (let* ([left? (< col 6)]
         ...
         [diode (cond ;;[(> diode 54) (- diode 5)]
                      ;;[(> diode 51) (- diode 6)]
                      [true diode])]
         [net-col (if left? col (- col 1))]
         [diode-net `(net ,(+ 17 diode)
...
(define edge-cuts
  (for/list [(s '([31 22] [84 22]  [141 30] [127 30] [185 22] [237  22] [253 111] [164 130] [104 130] [16 111]))
             (e '([84 22] [127 30] [185 22] [141 30] [237 22] [253 111] [164 130] [104 130] [16  111] [31 22]))]
